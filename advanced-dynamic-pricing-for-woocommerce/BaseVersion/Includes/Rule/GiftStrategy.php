...
class GiftStrategy
{
    ...
    public function addCartItemGifts(&$cart, $collection)
    {
        ...
        if ($rule->getItemGiftStrategy() === $rule::BASED_ON_LIMIT_ITEM_GIFT_STRATEGY) {
            ...
            $rule->getItemGiftStrategy() === $rule::BASED_ON_SUBTOTAL_ITEM_EXCL_TAX_GIFT_STRATEGY
            || $rule->getItemGiftStrategy() === $rule::BASED_ON_SUBTOTAL_ITEM_INCL_TAX_GIFT_STRATEGY
        ) {
            if ($rule->getItemGiftSubtotalDivider()) {
                $inclTax = $rule->getItemGiftStrategy() === $rule::BASED_ON_SUBTOTAL_ITEM_INCL_TAX_GIFT_STRATEGY;
                $tmpCart = clone $cart;
                foreach ($collection->get_items() as $item) {
